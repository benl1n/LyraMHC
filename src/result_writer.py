...
def weeekly_result_writer(result_dict, cfg):
    ...
    """
    working_dir = HydraConfig.get().runtime.output_dir
    out_file_path = os.path.join(working_dir, 'weekly_result.txt')
    _, test_file, _ = get_data(cfg.train.name,
                                cfg.train.task,
                                cfg.train.data_path)
    ...
    with open(config.test_file) as in_file:
        for line_num, line in enumerate(in_file):
            ...
            if line_num == 0:
                ...
                uid = '{iedb_id}-{hla_a}-{peptide}'.format(
                    iedb_id=iedb_id,
                    hla_a=hla_a,
                    # hla_b=hla_b,
                    peptide=peptide,
...
def get_srcc(real, pred):
    ...
    return pearsonr(pred, real)


def get_auc(real, pred):
    """
    """
    # all pred are ic50, neg them to get real correlation
    # pred = [-x for x in pred]
...
def write_metrics_file(result_file, config):
    ...
    for record, info in result_info.items():
        ...
        out_str = '{}\t{}\t{}\t{}'.format(
            iedb_id, allele, pep_length, count
        )
        ...
        for method_name in METHOD_LIST:
            ...
            if len(pred_vals) != count:
                ...
                srcc = get_srcc(label_values, pred_vals)
                auc  = get_auc(label_values, pred_vals)
...
def get_weekly_result_info_dict(result_file):
    ...
    with open(result_file) as in_file:
        for line_num, line in enumerate(in_file):
            ...
            #measure_type = info[4]
            pep_len = len(info[1])
            measure_value = float(info[4])

            record_id = '{}-{}-{}'.format(iedb_id, full_allele, pep_len)
