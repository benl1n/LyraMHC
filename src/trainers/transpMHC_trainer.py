...
class TranspMHC_Trainer(BaseTrainer):
    ...
    def train(self, checkpoints_dir, fold, train_loader, val_loader):
        ...
        warmup_epochs = cfg.dataset.train.get('warmup_epochs', 15)  # 保持激进的 15 轮

        optimizer = optim.Adam(model.parameters(), lr=cfg.dataset.train.start_lr, weight_decay=cfg.dataset.train.weight_decay)
        scheduler = lr_scheduler.ReduceLROnPlateau(
            optimizer, threshold=cfg.dataset.train.loss_delta, patience=cfg.dataset.train.scheduler_patience, cooldown=cfg.dataset.train.cooldown, min_lr=cfg.dataset.train.min_lr,
        ...
        early_stop_callback = EarlyStopCallBack(patience=cfg.dataset.train.callback_patience, delta=cfg.dataset.train.loss_delta)
        ...
        if cfg.train.task == "pMHC":
            for epoch in tqdm(range(cfg.dataset.train.epochs), desc=f"Training Fold {fold}"):
                ...
                # --- 1. 获取并记录当前真实 LR ---
                # 不再需要手动计算，完全信任 Scheduler
                current_lr = optimizer.param_groups[0]['lr']

                # 根据阶段打印不同日志，方便你监控
                if epoch < warmup_epochs:
                    log_msg = f"[Warmup] Climbing -> {current_lr:.2e}"
                else:
                    log_msg = f"[Cosine] Decaying -> {current_lr:.2e}"

                log_to_file("LR-Status", f"Epoch {epoch}: {log_msg}")
                ...
                for hla_list, pep_list, label, sample in train_loader:
                    ...
                    # --- 魏征死命令：必须用 Norm 裁剪！---
                    # 之前的 clip_grad_value_ 是错误的，它会改变梯度方向
        ...
        if cfg.train.task == "TCR":
            ...
            for epoch in tqdm(range(cfg.dataset.train.epochs), desc=f"Training Fold {fold}"):
                ...
                last_path = get_model_save_path(cfg, checkpoints_dir, self.cfg.dataset.params.model_count - 1, cfg.dataset.train.name, prefix="last")
                torch.save(model.state_dict(), last_path)
